...
def verify_state(solenoid_num, state, timeout=SENSOR_VERIFICATION_TIMEOUT):
    ...
    while time.time() - start_time < timeout:
        ...
        result = 'Pass' if (default or actuate) else 'Checking...'
        log_to_gui(f"Verifying solenoid {solenoid_num+1}, state: {state}, result: {result}", "DEBUG")
        
        if state == 'default' and default:
            log_to_gui(f"Solenoid {solenoid_num+1} verified in default position", "INFO")
            return True
        elif state == 'actuate' and actuate:
            log_to_gui(f"Solenoid {solenoid_num+1} verified in actuated position", "INFO")
            return True
...
def auto_mode():
    ...
    def cycle():
        ...
        if stop_auto:
            log_to_gui("Auto mode cycle stopped", "INFO")
            return
        log_to_gui("Starting new auto cycle", "INFO")
        execute_sequence_step_with_feedback(0, run_stepper_motor_then_cycle)

    cycle()

def run_stepper_motor_then_cycle():
    """Run stepper motor, then restart cycle"""
    run_stepper_motor(callback=lambda: root.after(int(POST_MOTOR_DELAY * 1000), restart_cycle))

def restart_cycle():
    """Restart the auto mode cycle"""
    if not stop_auto:
        log_to_gui("Restarting auto cycle", "INFO")
        log_to_gui("-" * 50, "INFO")
        execute_sequence_step_with_feedback(0, run_stepper_motor_then_cycle)
    else:
        log_to_gui("Auto mode stopped - not restarting cycle", "INFO")

def stop_auto_mode():
    """Stop automatic operation mode"""
    global stop_auto
    stop_auto = True
    log_to_gui("Auto Mode Stopped", "WARNING")
    update_gui()
...
def run_stepper_motor(callback=None):
    ...
    def step_once():
        ...
        global last_sensor_state, motor_running
...
def reset_system():
    ...
    global stop_auto, manual_mode_active, motor_running
    stop_auto = True
    manual_mode_active = False
    motor_running = False
    errors.clear()
...
def emergency_stop():
    ...
    errors.append(error_msg)
    log_to_gui(error_msg, "ERROR")
    log_to_gui("All operations halted", "ERROR")
    update_gui()
...
log_to_gui("System initializing...", "INFO")

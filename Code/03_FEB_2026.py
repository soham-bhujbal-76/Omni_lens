...
def execute_sequence_step(step):
    solenoid_states[:] = sequence[step]  # Update solenoid states as per the step
    write_solenoids()
    update_gui()

def auto_mode():
    global stop_auto, manual_mode_active
    manual_mode_active = False  # Ensure manual mode is off in auto mode

    def actuate_next_step(step):
        global stop_auto
        if stop_auto:
            return
        if step < len(sequence):
            execute_sequence_step(step)
            root.after(400, lambda: actuate_next_step(step + 1))  # Wait 2 seconds before next step
        else:
            run_stepper_motor() # Run stepper motor after all solenoids actuated
            root.after(400, lambda: actuate_next_step(0))  # Start next cycle
